...
- Destination registries (chests and landing pads) are runtime-only, rebuilt from world entities at game start, and kept in memory via `State.get_chests()`/`State.get_landing_pads()`. They are never persisted to `storage`, avoiding stale state and save/load stability issues.
