...
- `state.lua` — Persistent Demands (including the primary `source` field), Shipments, Demand-owned pad sections, tracked construction entities, dirty queues, bootstrap state, route preferences, platform options, fleet snapshots, return cargo, GUI state, history, and schema migration. Legacy `requests`/`request_by_key`/`next_request_id` aliases are read during migration then dropped so only `demands`/`demand_by_key`/`next_demand_id` persist. Destination registries (chests, landing pads) are runtime-only, not persisted. `State.for_each_sorted_shipment` iterates a demand's shipments in sorted id order for deterministic cleanup.
- `util.lua` — Pure item, signal, surface, route, platform, destination grouping, formatting, sorting, GPS, ghost, sprite helpers, blocklist-based `is_shippable`, and `resolve_cargo_endpoint` for Factorio 2.1 cargo pod destination/origin resolution
